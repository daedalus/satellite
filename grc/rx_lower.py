...
class rx_lower(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, fft_len=2048, fllbw=0.002, frame_sync_verbosity=1, freq=0, freq_rec_alpha=0.001, gain=40, ip='', loopbw=100, loopbw_0=100, poll_rate=100, port=5201):
        ...
        def _est_cfo_hz_probe():
            while True:
                val = self.tuning_control.update_nco_freq(self.mods_ffw_coarse_freq_rec_0.mods_runtime_cfo_ctrl_0.get_cfo_estimate(), self.mods_ffw_coarse_freq_rec_0.mods_runtime_cfo_ctrl_0.get_rf_center_freq(), [self.rtlsdr_source_0.get_center_freq() if hasattr(self, 'rtlsdr_source_0') else freq])
                try:
...
def argument_parser():
    ...
    parser.add_option(
        ...
        help="Set Destination Port [default=%default]")
    return parser


def main(top_block_cls=rx_lower, options=None):
    if options is None:
